...
class TestThreadWithMocking:
    ...
    @pytest.mark.asyncio
    async def test_thread_function_call_execution(self):
        """Test thread with function call and tool execution."""
        ...
        class TestTool(Tool):
            ...
            async def handle(self, message: str = "default", **kwargs) -> ToolResponse:
                return ToolResponse(output=f"Processed: {message}")
        ...
        # Create follow-up text response
        text_response = MockResponse([
            MockResponseOutputItem(item_type="text", content="Function executed successfully!")
        ])
        ...
        with patch('fast_agents.thread.AsyncOpenAI') as mock_openai_class:
            ...
            # Should have the function call response and potentially the follow-up
            assert len(responses) >= 1
            assert responses[0].name == "test_function"
            assert responses[0].arguments == {"message": "test input"}
